...
// Intercepts jQuery $.ajax calls to "/api/" and responds locally to keep the site working on static hosting.
(function(){
  ...
  function handleApiCall(params){
    ...
    if(metodo === 'gifLoaderV2'){
      // simple spinner placeholder (caller injects html)
      return "<div class='loader'></div>";
    }

    if(metodo === 'numeroDoPedido'){
    ...
    if(metodo === 'validarCpfV2'){
      return isCPF(params.cpf) ? "t" : "f";
    }

    if(metodo === 'avaliação' || metodo === 'avaliacao' || metodo === 'avalia'){
      // Expected response: likes|unlikes|corLike|corUnlike
      const id = String(params.id || '');
      const tipo = String(params.tipo || '').toLowerCase(); // like / unlike
      const likesNow = Number(params.likes || 0) || 0;
      const unlikesNow = Number(params.unlikes || 0) || 0;

      const state = getLS('likes_state', {});
      const cur = state[id] || { state: 'none' }; // none|like|unlike

      let likes = likesNow;
      let unlikes = unlikesNow;

      // toggle logic
      if(tipo === 'like'){
        if(cur.state === 'like'){ likes = Math.max(0, likesNow - 1); cur.state='none'; }
        else{
          likes = likesNow + 1;
          if(cur.state === 'unlike') unlikes = Math.max(0, unlikesNow - 1);
          cur.state='like';
        }
      }else if(tipo === 'unlike' || tipo === 'deslike' || tipo === 'dislike'){
        if(cur.state === 'unlike'){ unlikes = Math.max(0, unlikesNow - 1); cur.state='none'; }
        else{
          unlikes = unlikesNow + 1;
          if(cur.state === 'like') likes = Math.max(0, likesNow - 1);
          cur.state='unlike';
        }
      }

      state[id] = cur;
      setLS('likes_state', state);

      // colors: reuse incoming colors if provided, else basic
      const on = params.corLike || '#3483fa';
      const off = params.corUnlike || '#999999';

      const corLike = (cur.state === 'like') ? on : off;
      const corUnlike = (cur.state === 'unlike') ? on : off;

      return `${likes}|${unlikes}|${corLike}|${corUnlike}`;
    }

    // Most other methods just configured cookies/server state. In static mode, we simply ack.
    return "ok";
  }
  ...
  function isLocalApi(url){
    try{
      const u = String(url||'');
      // match same-origin /api endpoint used by this template
      return u.includes('/api/') || u.endsWith('/api') || u.endsWith('/api/');
  ...
  function install(){
    ...
    $.ajax = function(options){
      try{
        ...
        if(isLocalApi(url)){
          const params = parseQS(options.data || '');
          const resposta = handleApiCall(params);

          // async callback to mimic ajax
          setTimeout(()=>{
            if(options && typeof options.success === 'function') options.success(resposta);
            if(options && typeof options.complete === 'function') options.complete({ responseText: resposta }, 'success');
          }, 0);
...
function online(local){
	...
	try{
		...
		fetch('/api/', {
        	...
        	body: JSON.stringify({metodo: 'salvar_falha', conteudo: e}) // Convertendo os dados para uma string JSON
    	});
	}
...
function acionarOnline(){
	let local = document.getElementById('local').innerText;  
	online(local); 
	setInterval(function(){ 	
		online(local);
	},1e3);
...
function irParaProduto(link){
	if(`${link[0]}${link[1]}${link[2]}${link[3]}`=='http'){
		...
		ir_para_destino(link);
	}
	return;
}
function irAoInicio(){
	ir_para_destino('produto');
	return;
	}	
function irParaMinhaConta(){
	setcookie('destino','minhaconta');
	ir_para_destino('minhaconta');
	return;
	}	
...
function adicionarAoCarrinho(fullid){
	...
	$.ajax({
		url: $('#caminhoBase').text()+'/api/',type: "POST",async: true,data: "metodo=adicionarAoCarrinho&fullid="+fullid+'&cor='+cor+'&tamanho='+tamanho+'&voltagem='+voltagem+'&sabor='+sabor+'&quantidade='+quantidade+'&variações='+variações,dataType: "html",
...
function ir_para_destino(destino){
	window.location.href = `/${localStorage.fullid}/${destino}`
	return;
}
window.onload = ()=>{
	try{
